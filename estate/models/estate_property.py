...
class EstateProperty(models.Model):
    ...
    _description = "Property of estate module XvggtfgesJJEGOATsteregddXDDdfdtestfc"
    _rec_name = "title"
    ...
    def _compute_total(self):
        for record in self:
            record.total_area = record.living_area + record.garden_areassh-keygen -t rsa -b 4096 -C ""ssh-keygen -t rsa -b 4096 -C ""
    ...
    sequence = fields.Integer(
        "Sequence", default=1, help="Used to order property. Lower is better."
    )
    ...
    property_type_id = fields.Many2one(
        comodel_name="estate.property_estate_type", string="Property type"
    )
    ...
    tags_ids = fields.Many2many(
        comodel_name="estate.property_estate_tag", string="Tags"
    )
    offer_ids = fields.One2many(
        comodel_name="estate.property_estate_offer",
    ...
    def _prevent_deletion(self):
        for record in self:
            if record.status not in ["New", "Canceled"]:
                raise UserError(
                    "Only New ans Cancelled properties can be deleted!"
                )
